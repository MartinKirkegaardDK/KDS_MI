...
def get_activations(
    ...
    for ind, (text, label) in enumerate(tqdm(loader)):
        ...
        attn_mask = tokenized.attention_mask.flatten().detach().cpu() # # flattening [batch, pad_size, ...] to [tokens, ...]
        label = label.unsqueeze(-1).expand(-1, tokenized.attention_mask.shape[1]).flatten().detach().cpu() # [tokens]
        for layer in layers:
            for hook_address in hook_addresses:
                to_add = extracted[hook_address.layer(layer)][0].view(-1, ModelConfig.hidden_size(model)).detach().cpu() # [tokens, hidden_size]

                # add to dataset
                activation_ds[hook_address.layer(layer)].add_with_mask(to_add, label, attn_mask, sampling_prob=0.005)
